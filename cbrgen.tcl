...
proc create-pong-connection { src dst } {
...
proc create-cbr-connection { src dst } {
	global rng cbr_cnt opt timenow
	
	set stime [$rng exponential 0.5]
    set timenow [ expr $timenow + $stime]
	puts "#\n# $src connecting to $dst at time $stime\n#"

	##puts "set cbr_($cbr_cnt) \[\$ns_ create-connection \
		##CBR \$node_($src) CBR \$node_($dst) 0\]";
	puts "set pong1_($cbr_cnt) \[new Agent/Pong\]"
	puts "\$ns_ attach-agent \$node_($src) \$pong1_($cbr_cnt)"
	puts "set pong2_($cbr_cnt) \[new Agent/Pong\]"
	puts "\$ns_ attach-agent \$node_($dst) \$pong2_($cbr_cnt)"
	
	puts "\$ns_ connect \$pong1_($cbr_cnt) \$pong2_($cbr_cnt)"
	
	puts "\$ns_ at $timenow \"\$pong1_($cbr_cnt) send\"" 
